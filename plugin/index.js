...
plugin.didReceiveGlobalSettings = ({ payload: { settings } }) => {
    ...
    if (settings && settings.debugPort) {
        ...
        if (!isNaN(savedPort) && savedPort >= 1 && savedPort <= 65535) {
            ...
            yandexMusicLauncher.setDebugPort(savedPort);
...
const TEXT_BUTTON_IMAGE = 'imgs/text-bg';
...
plugin["ym-cover"] = new Actions({
    ...
    async keyUp({ context, payload }) {
        log.info('Нажата кнопка обложки, проверяем и запускаем Яндекс Музыку...');
        sendLogToPropertyInspector('Проверка состояния Яндекс Музыки...', 'info');
        
        try {
            // Просто проверяем и запускаем - без анимаций, быстро
            const success = await yandexMusicLauncher.ensureYandexMusicRunning();
            
            if (success) {
                sendLogToPropertyInspector('✅ Яндекс Музыка запущена с правильными параметрами', 'info');
                log.info('Яндекс Музыка готова к работе');
                
                // Обновляем соединение CDP в фоне (не блокируем)
                (async () => {
                    try {
                        await yandexMusic.disconnect();
                        await new Promise(resolve => setTimeout(resolve, 300));
                        await yandexMusic.connect();
                        sendLogToPropertyInspector('✅ CDP соединение обновлено', 'info');
                    } catch (cdpError) {
                        log.warn('Ошибка при обновлении CDP соединения:', cdpError);
                    }
                })();
                
                // Обновляем обложку в фоне (не блокируем)
                checkCoverState().catch(err => {
                    log.error('Ошибка при обновлении обложки:', err);
                });
                
                // Не показываем галочку - просто работаем тихо в фоне
            } else {
                sendLogToPropertyInspector('❌ Не удалось запустить Яндекс Музыку. Проверьте путь к приложению.', 'error');
                log.error('Не удалось запустить Яндекс Музыку');
                plugin.showAlert(context);
            }
        } catch (error) {
            log.error('Ошибка при проверке/запуске Яндекс Музыки:', error);
            sendLogToPropertyInspector(`❌ Ошибка: ${error.message}`, 'error');
            plugin.showAlert(context);
        }
    }
});
...
plugin["ym-track-info"] = new Actions({
    ...
    async _willAppear({ context, payload }) {
        ...
        plugin.setImage(context, TEXT_BUTTON_IMAGE);
            ...
            switch (command) {
                ...
                case 'changePort':
                    if (message.payload.port) {
                        ...
                        yandexMusicLauncher.setDebugPort(newPort);
...
plugin.sendToPropertyInspector = function(payload, context, action) {
    ...
    if (!action) {
        ...
        if (!action) {
            action = 'ru.yandex.music.ym-cover';
        }
    }
